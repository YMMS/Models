...
使用指南:
    ...
    5. 5-way 1-shot mini imagenet:
    
    python main.py --datasource=miniimagenet --metatrain_iterations=60000 --meta_batch_size=4 --update_batch_size=1 --update_lr=0.01 --num_updates=5 --num_classes=5 --logdir=logs/miniimagenet1shot/ --num_filters=32 --max_pool=True
    #######################################################################################

    6. 5-way 5-shot mini imagenet:
    
    python main.py --datasource=miniimagenet --metatrain_iterations=60000 --meta_batch_size=4 --update_batch_size=5 --update_lr=0.01 --num_updates=5 --num_classes=5 --logdir=logs/miniimagenet5shot/ --num_filters=32 --max_pool=True
    #######################################################################################

    评估时，设置 '--train=False' 以及 '--test_set=True'

    Omniglot 以及 miniimagenet 训练需要online的数据集（数据集是动态构造，不是提前准备好保存在硬盘上的）, 将总数据集放置到对应的数据文件夹, 并参阅文件夹下的数据预处理脚本
"""
...
flags.DEFINE_string('datasource', 'sinusoid', '数据源选择：可选值为sinusoid/omniglot/miniimagenet')
flags.DEFINE_integer('num_classes', 5, '分类任务的类别数(e.g. 5-way classification).')
# oracle means task id is input (only suitable for sinusoid)
flags.DEFINE_string('baseline', None, 'oracle, or None')

## Training options
flags.DEFINE_integer('pretrain_iterations', 0, '任务pre-train的迭代轮数，默认值为0, 即不进行预训练.')
flags.DEFINE_integer('metatrain_iterations', 15000, 'metatraining的迭代轮数(omniglot设置为15k, sinusoid设置为50k).')
...
flags.DEFINE_bool('log', True, '如果为False, 则不进行summaries相关操作, 以用于代码调试.')
flags.DEFINE_string('logdir', '/tmp/data', 'summaries 和 checkpoints 的文件夹路径')
flags.DEFINE_bool('resume', True, 'resume training if there is a model available')
flags.DEFINE_bool('train', True, 'True to train, False to test.')
flags.DEFINE_integer('test_iter', -1, 'iteration to load model (-1 for latest model)')
flags.DEFINE_bool('test_set', False, '如果设置为true，则只有meta train/meta test两个类别集合, 如果为False则有meta valid类别集合.')
...
def train(model, saver, sess, exp_string, data_generator, resume_itr=0):
    ...
    if FLAGS.log:
        "如果不做日志（即代码调试阶段），"
        "则不创建用于summary的train_writer"
        train_writer = tf.summary.FileWriter(FLAGS.logdir + '/' + exp_string, sess.graph)
    print('已完成初始化, 开始训练.')
    ""
    prelosses, postlosses = [], []
    "当前任务的类别数，对于正弦曲线拟合num_classes=1"
